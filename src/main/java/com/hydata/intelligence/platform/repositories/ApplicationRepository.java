...
public interface ApplicationRepository extends JpaRepository<Application, Integer> {
	...
	List<Application> findByProduct_idAndLikeName1(Integer product_id,String name);
	
	@Query("select a from Application a where a.productId = ?1 and a.applicationType = 1 and a.name like concat('%' ,?2,'%')")
	List<Application> findByProduct_idAndLikeName2(Integer product_id,String name);
